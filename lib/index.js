...
function beatrix({ config, outputPath, typefacesDirPath }) {
  if (!config) {
  ...
  const TYPEFACES_DIR = path.resolve(process.cwd(), typefacesDirPath);
  ...
  const fonts = typefaces.reduce((acc, name) => {
    acc[name] = fs.readdirSync(`${TYPEFACES_DIR}/${name}`)
      .filter(x => !x.startsWith('.') && (x.endsWith('.ttf') || x.endsWith('.otf')));
  ...
  Object.keys(fonts).forEach(key => {
    ...
    const fontDistDir = path.join(OUTPUT_PATH, key);
    const fontSrcDir = path.join(TYPEFACES_DIR, key);
    ...
    fontFiles.forEach(file => {
      ...
      console.log(`TTF subset. Stat: ${prevTTFSize} Kb → ${currentTTFSize} Kb (−${Math.floor(100 - currentTTFSize / prevTTFSize * 100)}%).`);
...
`);
  ...
  fs.writeFileSync(path.join(OUTPUT_PATH, 'index.html'), template);
