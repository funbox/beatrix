...
const fonts = typefaces.reduce((acc, x) => {
  acc[x] = fs.readdirSync(`${TYPEFACES_DIR}/${x}`).filter(x => !x.startsWith('.'));
  return acc;
}, {});
...
const charactersInfo = CHARACTERS.reduce((acc, x) => {
  const [startHex, endHex] = x.split('-');

  if (!endHex) {
    const dec = parseInt(x, 16);
    return acc.concat([{
      hex: `U+${x}`,
      dec,
...
function convert(src, dest, characters) {
  const args = [
    ...
    // drop all features: 'calt', 'ccmp', 'clig', 'curs', 'dnom', 'frac',
    // 'kern', 'liga', 'locl', 'mark', 'mkmk', 'numr', 'rclt', 'rlig', 'rvrn',
    // etc
    '--layout-features=""',
